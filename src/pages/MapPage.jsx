...
function MapPage({ user }) {
  ...
  function addPinToMap(map, pin) {
    ...
    const icon = createColorPin(color)
    ...
    <>
      ...
      {riderCard && (
        <div style={{
          ...
          {/* 閉じるボタン */}
          <button
            onClick={() => { setRiderCard(null); setRiderProfile(null) }}
            style={{
              position: 'absolute',
              top: '12px',
              right: '12px',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            ...
            <div>
              <div style={{ fontWeight: 'bold', fontSize: '15px' }}>
                {riderProfile === null
                  ? '読み込み中...'
                  : (riderProfile.username || 'ライダー')}
              </div>
              {riderProfile?.bike_model && (
                <div style={{ fontSize: '12px', color: '#aaa', marginTop: '2px' }}>
                  🏍️ {riderProfile.bike_model}
                </div>
              )}
              {/* 日時は1行だけ表示 */}
              <div style={{ fontSize: '12px', color: '#888', marginTop: '2px' }}>
                {new Date(riderCard.created_at).toLocaleString('ja-JP', {
                  month: 'numeric',
                  day: 'numeric',
                  hour: '2-digit',
                  minute: '2-digit',
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              {PIN_TYPES.map((type) => {
                ...
                return (
                  <button
                    key={type.key}
                    onClick={() => setPinType(type.key)}
                    style={{
                      flex: 1,
                      padding: '8px 4px',
                      borderRadius: '8px',
                      border: isSelected
                        ? `2px solid ${PIN_COLORS[type.key]}`
                        : '2px solid rgba(255,255,255,0.15)',
                      background: isSelected
                        ? `${PIN_COLORS[type.key]}22`
                        : 'transparent',
                      color: 'white',
                      cursor: 'pointer',
                      fontSize: '11px',
